...
student_score <- round(pmin(pmax(rnorm(100, 75, 20),0),100))

student_int <- sample(c("Sport", "Music", "Nature", "Literature"),100, replace=TRUE)

student_df<-data.frame(student_int,student_score, as.is=TRUE)
student_df %>%
  mutate(scores=as.numeric(student_score))
class(student_df$student_score)
...
```{r}
hist(as.numeric(student_df$student_score), breaks = 3)
...
cut(as.numeric(student_df$student_score),breaks = c(0, 50, 60, 70, 80, 90, 100), letters[6:1])
...
hist(as.numeric(student_df$student_score), breaks = 4, col=mypalette, xlab = "Greens Sequential")
...
#Create a boxplot of total enrollment for three schools
boxplot(student_df$student_score ~ student_int, col=mypalette)
...
```{r}
logins <- round(pmin(pmax(rnorm(100, 12.5, 1),0),25))
length(logins)
student_df$logins <- logins
...
```{r}
plot(student_df$logins,student_df$student_score)
...
plot(iris$Sepal.Length,iris$Petal.Length)#this one looks pretty good
plot(iris$Sepal.Length,iris$Petal.Width)#Looks pretty good
plot(iris$Sepal.Width,iris$Petal.Length)
plot(iris$Sepal.Width,iris$Petal.Width)
plot(iris$Petal.Length,iris$Petal.Width)#Looks pretty good
